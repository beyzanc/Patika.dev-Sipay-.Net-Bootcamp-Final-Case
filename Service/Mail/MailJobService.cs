...
namespace ResiPay.Service.Mail
{
    public class MailJobService : IMailJobService
    {
        ...
        public MailJobService(IMapper _mapper, ResiPayDbContext dbContext, ITokenService tokenService, IBillService billService)
        {
            this.mapper = _mapper;
            ...
            try
            {
                ...
                for (int i = count - 1; i >= 0; i--)
                {
                    ...
                    var request = new MailRequestModel
                    {
                        ...
                        Body = $"Dear {bill.User.Name},\n\nWe hope this message finds you well. We kindly remind you about your {bill.BillType} invoice, which is due for payment on {bill.DueDate.ToShortDateString()}. The total amount to be paid is {bill.BillAmount} TL. We would like to kindly request you to settle the payment by the specified due date.\n\nThank you for your attention. Should you have any questions or require further assistance, please don't hesitate to reach out to us!\n\nBest regards,\n\nThe ResiPay Team"
                    };
                ...
                return new MailResponseModel { IsSuccess = true, Message = "Reminders sent successfully." };
            }
            catch (Exception ex)
            {
                return new MailResponseModel { IsSuccess = false, ErrorMessage = ex.Message };
            }
