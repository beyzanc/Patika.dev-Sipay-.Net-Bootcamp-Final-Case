...
namespace ResiPay.Service.Bill
{
    public class BillService : IBillService
    {
        ...
        public Base<BillViewModel> GetPaidBillsForUser(int id)
        {
            ...
            var bills = dbContext.Bill
                .Where(b => b.IsPaid && b.UserId == id)
                .OrderBy(a => a.Month)
                .ThenBy(a => a.UpdateDate);
        ...
        public Base<BillViewModel> GetMonthlyBills(int month, int year)
        {
            ...
            var notPaidBills = bills
                .Where(i => !i.IsPaid); 

            var totalPaidAmount = paidBills
                .Sum(i => i.BillAmount); 

            var totalNotPaidAmount = notPaidBills
                .Sum(i => i.BillAmount);
        ...
        public Base<BillViewModel> Update(BillUpdateModel billToUpdate)
        {
            ...
            var model = dbContext.Bill
                .SingleOrDefault(b => b.Id == billToUpdate.Id);
        ...
        public Base<BillViewModel> AssignBill(BillInsertModel newBill)
        {
            ...
            var isExist = dbContext.Bill
                .Any(x => x.DueDate == newBill.DueDate
        ...
        public Base<BillViewModel> PayBill (int id, string type, string cardNumber)
        {
            ...
            var bill = dbContext.Bill
                .FirstOrDefault(x => x.UserId == id && x.BillType == type && !x.IsPaid );
        ...
        public Base<BillViewModel> PayBills(int id, string type, string cardNumber)
        {
            ...
            var unpaidBills = dbContext.Bill
                .Where(x => x.UserId == id && x.BillType == type && !x.IsPaid).ToList();
